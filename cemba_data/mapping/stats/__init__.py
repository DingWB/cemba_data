...
def mhap(bam_path=None,cpg_path="~/Ref/hg38/annotations/hg38_CpG.gz",
		 output="test.mhap",method=None):
	"""
	convert bam into .mhap.gz, similar to:
		java -jar ${mHapSuite} convert --inputFile input_bam --cpgPath ${cpgPath} --outPutFile output.mhap.gz
		zcat output.mhap.gz | sort -k1,1 -k2,2n | bgzip > output_tmp.mhap.gz
		rm -f output.mhap.gz && mv output_tmp.mhap.gz output.mhap.gz
