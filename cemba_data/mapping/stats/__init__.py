...
def _cpg_position_generator(records):
	for line in records:
...
def reverse_comp(sequence):
	...
	return _seq
def mhap(bam_path,cpg_path):
	fbam = pysam.AlignmentFile(bam_path, 'rb')
	cpg = pysam.TabixFile(cpg_path) # positions is 1-based
	R=[]
	for read in fbam:
		...
		if read.has_tag('YZ'):
			ct_read=read.get_tag('YZ') == '+'
		elif read.has_tag('XG'):
			ct_read=read.get_tag('XG') == 'CT'
		else:
			raise ValueError(
		...
		cytosine=''
		start=read.pos #0-based
		for cigar in read.cigar:
			end=start+cigar[1] #1-based for end
			if cigar[0]!=0: # not equal to M, not matched
				start=end #0-based for start, next pos of this one
				continue
		...
		if not ct_read:  # CTOB,OB,R1?
			seq = read.seq + 'G'
			cytosine = ''.join([seq[i - read.pos] for i in positions])  # read.pos is 0-based, pos of C
		else:  # CTOT,OT,R2?
			cytosine = ''.join([read.seq[i - read.pos - 1] for i in positions])  # CG, position of G
		if cytosine == '':
			continue
		if ct_read:
			...
			# vector = cytosine.replace('G', '1').replace('A', '0')
			vector = ''.join(['1' if c == 'G' else '0' for c in cytosine])
		assert set(vector) in [{'0'},{'1'},{'0','1'}],f"read1:{read.is_read1},forward: {read.is_forward},ct_read: {ct_read}" # 1 for methylated, 0 for unmethylated
		strand='+' if read.is_forward else '-'
		# print(f"{chrom}\t{positions[0]}\t{positions[-1]}\t{vector}\t1\t{strand}")
