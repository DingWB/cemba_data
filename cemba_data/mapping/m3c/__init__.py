...
def _parse_bam(bam_path, output_path):
	...
	for read in dfh:
		...
		if '_' in line[0]:
			...
			if ':' in suffix:
				split_st = suffix.split(':')[0] # 1 for R1 and 2 for R2, strand?
				# typically, line[0]: lh00170:124:22HC72LT3:1:1101:14684:6736_2:N:0:TGACTCGCGT+ATTCCTCTGG-l
				# split_st=2
			elif '/' in suffix: # downloaded from SRA
				split_st = suffix.split('/')[1].split('-')[0]
				# if fastq is downloaded from SRA, for example, line[0]: SRR22760858.1_1/1-l
				# split_st=1/1-l, would get error in split_dict[split_st]
			else:
				raise ValueError(f'Invalid read name: {line[0]} and suffix: {suffix}')
			if line[0][-2:] == '-l':
				split_st += '-1'
			elif line[0][-2:] == '-r':
				split_st += '-2'
